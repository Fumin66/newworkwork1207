    ...
    switch(mode){
        ...
        case 'checkErase':
            ...
            if(eraseInfo){
                ...
                combinationCount++;
                //得点を計算する
                ScreenOrientation.calculateScore(combinationCount, eraseInfo.piece, eraseInfo.color);
                Stage.hideZenkeshi();
            }
            else{
                if(Stage.puyoCount === 0 && combinationCount > 0){
                    //全消しの処理をする
                    Stage.showZenkeshi();
                    ScreenOrientation.addScore(3600);
                }
                combinationCount = 0;
                //消せなかったら、新しいぷよを登場させる
                mode = 'newPuyo'
