class Player {
    ...
    static initialize() {
        ...
        const gesture = (xs, ys, xe, ye) => {
            ...
            if (Math.abs(horizonDirection) < Math.abs(vertificalDirection)) {
                //縦方向
    ...
    static createNewPuyo() {
        //ぷよぷよが置けるかどうか、一番上の段の左から三つ目を確認する。
        if (StaticRange.board[0][2]) {
            //空白でない場合は新しいぷよを置けない
            return false;
        }
        //新しいぷよの色を決める
    ...
    static falling(isDownPressed) {
        ...
        if (y + 1 >= Config.stageRows || Stage.board[y + 1][x] || (y + dy + 1 >= 0 && (y + dy + 1 >= Config.stageRows || Stage.board[y + dy + 1][x + dy]))) {
            isBlocked = ture;
        }
        if (!isBlocked) {
            //下にブロックがないなら自由落下をしてよい。プレイヤー操作中の自由落下処理をする
    ...
    static playing(frame) {
        ...
        if (this.keyStatus.right || this.keyStatus.left) {
            ...
            if (rotation === 0) {
                ...
                //上から左に回すときに、左にブロックがあれば、右に移動する必要があるのでまず、確認する
                ...
                //右にずれる必要がある時、右にもブロックがあれば回転できないので確認する。
                if (cx === 1) {
                ...
                //左から下に回すときには、自分の下か左下にブロックがあれば1個上に引き上げる。まず、下を確認する。
                ...
                //左にずれる必要ある時、左にもブロックがあれば回転できないので確認する
                if (cx === -1){
                    if(y + 1 < 0 || x - 1 < 0 || x - 1 >= Config.stageCols || Stage.board[y + 1][x - 1]){
                        if(y + 1 >= 0){
                            //ブロックがある。回転できなかった。
